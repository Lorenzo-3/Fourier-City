...
const cubeMap = cubeTextureLoader.load(['img/stars.jpg', 'img/stars.jpg', 'img/stars.jpg', 'img/stars.jpg', 'img/stars.jpg', 'img/stars.jpg'],
    function (cubeMap) {
        const pmremGenerator = new THREE.PMREMGenerator(renderer);
        envMap = pmremGenerator.fromCubemap(cubeMap).texture;
        pmremGenerator.dispose();

        // Now that envMap is ready, assign it to all your materials
...
// Main directional light (simulating sun / overhead)
const dirLight = new THREE.DirectionalLight(0xfff5e6, 1.2);
dirLight.position.set(10, 20, 5);
dirLight.castShadow = true;
dirLight.receiveShadow = false; // shadows optional for performance
...
// 1. Black floor plane (receives shadows)
...
// 2. White grid helper (floats slightly above to avoid z‑fighting)
const gridHelper = new THREE.GridHelper(300, 75, 0x000000, 0x000000);
gridHelper.position.y = 0.01;   // tiny offset
...
loader.load('models/table.obj', (object) => {
    const tableMaterial = new THREE.MeshStandardMaterial({
        ...
        roughness: 0.2,
        metalness: 0.1,
        envMap: envMap,           // may be null initially, will be updated later
        envMapIntensity: 0.8
    });
    ...
    // If the envMap has already loaded, apply it now
    if (envMap) {
        object.traverse((child) => {
            if (child.material) {
                child.material.envMap = envMap;
...
loader.load('models/table_wireframe.obj', (object) => {
    const wireframeMaterial = new THREE.MeshStandardMaterial({ color: 0x0000000, 
        wireframe: true, 
        roughness: 0.2, 
        metalness: 0.1, 
        envMap: envMap, 
...
function createButton(position, name, color, text, isBand = false, source = null, isMusic = false, isSelect = false) {
    ...
    material = new THREE.MeshStandardMaterial({ color: color, roughness: 0.25, metalness: 0, envMap: envMap, envMapIntensity: 0.6,});
