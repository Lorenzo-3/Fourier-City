...
import { normalizedValueToIndex } from './target-mapping.js';
import {
    buildInstancedGeometry,
    buildQuadWireframeGeometry,
    getNormalizeMatrix,
    loadText
} from './geometry.js';
import { initializeGroundFlowData, updateGroundFlowData } from './ground-flow.js';
import {
...
buildBaseBuildingColors(CONFIG, state.baseBuildingColors);
...
export function createSkylineVisualizer({ scene, camera, audio }) {
    initializeSkyline({ scene, camera, audio });
    return {
        update: updateSkyline,
        getTargetPosition: getSkylineTargetPosition,
        getBandCenterFrequency: (index) => getBandCenterFrequency(index, CONFIG),
        computeFilterMultipliers: (filterState) => computeFilterMultipliers(CONFIG, filterState),
        getBuildings: () => state.buildings
    };
}

function initializeSkyline({ scene, camera, audio }) {
    if (!scene || !camera || !audio) {
        throw new Error('createSkylineVisualizer requires a scene, camera, and audio source');
    }
...
async function ensureSkyline(scene) {
    ...
    try {
        ...
        const [model, objText] = await Promise.all([
            loader.loadAsync('assets/models/skyscraper/skyscraper.obj'),
            loadText('assets/models/skyscraper/skyscraper.obj')
        ]);
        const normalizeMatrix = getNormalizeMatrix(model, CONFIG.modelYawCorrectionDegrees);
        const geometry = buildInstancedGeometry(model, normalizeMatrix);
        const wireframeGeometry = buildQuadWireframeGeometry(
            objText,
            normalizeMatrix,
            CONFIG.modelYawCorrectionDegrees
        );
        const skyline = buildSkyline(geometry, wireframeGeometry);
        state.skyline = skyline;
        scene.add(skyline);
    } catch (error) {
        console.error('Fourier City skyline failed to load:', error);
    } finally {
        state.skylineLoading = false;
    }
}
...
function updateGroundFlow(energies, deltaSeconds) {
    ...
    state.groundFlowOverallEnergy = updateGroundFlowData({
        energies,
        deltaSeconds,
        config: CONFIG,
        energyData: state.groundFlowEnergyData,
        overallEnergy: state.groundFlowOverallEnergy
    });
